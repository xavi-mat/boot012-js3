...
console.log("\ncincoOMas");
const cincoOMas = (array) => array.filter((num) => num >= 5);

console.log("cincoOMas([3,6,8,2])", cincoOMas([3, 6, 8, 2]));
...
console.log("\nNúmero primo: pulsa el botón o ejecuta en la consola 'maybePrime()'.");

const maybePrime = () => {
    console.log("Introduce un número en el prompt...");
    const num = prompt("Introduce un número y te diré si es primo");
    if (isNaN(num)) {
        console.log("No es un número válido.");
        return;
    }

    const checkPrime = (num, check = 2) => {
        if (check > Math.sqrt(num)) { return true; }
        if (num % check === 0) { return false; }
        return checkPrime(num, check + 1);
    }
    console.log(`¿Es primo ${num}?`, checkPrime(num));
};


// Crea una función checkPassword con una variable tipo String que contenga una
...
console.log("\ncheckPassword: Click button or execute 'checkPasswordApp()'");

const password = '12345678';

const checkPasswordApp = () => {
    const checkPassword = (str) =>
        prompt("Input password") === str;
    const MAX_TRIES = 3;
    let tries = 0;
    let loggedIn = false;
...
const reverseCalculator = () => {
    ...
    if (isNaN(operand1)) {
        result += 'OPERAND1 IS INVALID. ';
    }
    if (isNaN(operand2)) {
        result += 'OPERAND2 IS INVALID. ';
    }
